...
- **This is the topmost layer.** The dependency arrow points one way — `vendra-console` → `vendra-reseller` → `vendra-store` → `laravel-docker-engine` — so nothing may depend on this package. A behaviour other panels also need belongs in the domain package, not here.
...
- `ContainerRuntimeHealth` uses only `vendra-store`'s `StorefrontContainerRuntime`, which resolves the configured `ContainerManager` driver. Keep runtime calls out of resource table rows; live inspection and logs are explicit operator reads on a deployment or store.
