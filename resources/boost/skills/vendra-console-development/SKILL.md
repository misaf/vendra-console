...
- Identifiers resolve separately: a username-only run does not fall back to the default email account, and both identifiers together must resolve to the same tenantless user or the run is rejected as mismatched. `user-grant` and `user-revoke` additionally require an identifier and reject a blank one; `user-create` and `user-password` fall back to `vendra-console.default_email` only when no identifier is given, and a blank `--email` is still rejected.
- `user-create` requires an explicit `--username`, passed to `CreateConsoleUserAction::execute($username, $email, $password)`; it never prompts for one. Validate with `UserRules::username()` plus `UserRules::unique('username')` for tenantless users; conflicts fail without suffixes or retries, and `ConsoleSeeder` explicitly uses `console`.
- `user-password` requires an active console row and confirms before replacing a password with a generated one (`--password` or `--force` skips it). `user-grant` reactivates a revoked console rather than adding a row, takes an optional `--password`, and reports an already-granted user without changing anything.
